...
class Steady_State_JC_Solutions(JC_Solutions):
    def __init__(self, drive_range, det_range):
        JC_Solutions.__init__(self, drive_range, det_range)
    ...
    def intracavity_photon_numbers(self):
        return [qt.expect(self.rho_ss(drive[0], det[0]), self.a.dag()*self.a) for drive in enumerate(self.drive_range) for det in enumerate(self.det_range)]

    def purities(self):
        return [(self.rho_ss(drive[0], det[0])**2).tr() for drive in enumerate(self.drive_range) for det in enumerate(self.det_range)]

class Time_Dependent_JC_Solutions(JC_Solutions):
    pass

vec = np.linspace(-8, 8, 200)


carmichael = Steady_State_JC_Solutions(5, [0, 1, 2])

# start = timer()
# iphnums = carmichael.intracavity_photon_numbers()
# end = timer()
