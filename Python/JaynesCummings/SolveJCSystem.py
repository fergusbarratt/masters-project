...
class DickeSystem(QuantumOpticsSystem):
    '''http://rsta.royalsocietypublishing.org/content/roypta/369/1939/1137.full.pdf'''
    def __init__(self, omega_cavity_range, omega_qubit_range, N_qubits, N_cavity_modes, c_op_params, g):
        QuantumOpticsSystem.__init__(self, c_op_params, g, N_cavity_modes, N_qubits)
        ...
        self.idqubits = qt.tensor(*tuple(self.idqubit for qubit in range(N_qubits)))
        self.sz = qt.tensor(self.idcavity, qt.tensor(*tuple(self.sz_bare for qubit in range(N_qubits))))
        self.sm = qt.tensor(self.idcavity, qt.tensor(*tuple(self.sm_bare for qubit in range(N_qubits))))
    ...
    def hamiltonian(self, det_index=0):
        H_bare = self.omega_cavity_range[det_index]*(self.a.dag()*self.a)+self.omega_qubit_range[det_index]*self.sz
...
class JaynesCummingsSystem(QuantumOpticsSystem):
    '''Jaynes Cummings System. Takes parameter ranges, builds hamiltonians at each value'''
    def __init__(self, drive_range, omega_qubit_range, omega_cavity_range, omega_drive_range, c_op_params, g, N):
        ...
        if isinstance(omega_qubit_range, numbers.Number):
            ...
            self.omega_qubit_range = np.array(omega_qubit_range)
        # set the important range data and name - for plotting against etc. drive_range by default, changes in block below
...
class SteadyStateDickeModel(DickeSystem):
    ...
    def __init__(self, omega_cavity_range, omega_qubit_range, N_qubits, N_cavity_modes, c_op_params, g):
        DickeSystem.__init__(self, omega_cavity_range, omega_qubit_range, N_qubits, N_cavity_modes, c_op_params, g)
    ...
    def qps(self, xvec, yvec, type='Q'):
        ...
        for det_index in range(len(self.omega_qubit_range)):
            if type != 'Q':
                qps.append(qt.wigner(self.rho(det_index).ptrace(0), xvec, yvec))
            else:
                qps.append(qt.qfunc(self.rho(det_index).ptrace(0), xvec, yvec))
        return  qps

    def intracavity_photon_numbers(self):
        return [qt.expect(self.rho(det[0]), self.a.dag()*self.a) for det in enumerate(self.omega_qubit_range)]

    def purities(self):
        return [(self.rho(det[0])**2).tr() for det in enumerate(self.omega_qubit_range)]

class SteadyStateJaynesCummingsModel(JaynesCummingsSystem):
    '''does steadystate things to the super jaynes cummings system'''
    def __init__(self, drive_range, omega_qubit_range, omega_cavity_range, omega_drive_range, c_op_params, g, N):
        JaynesCummingsSystem.__init__(self, drive_range, omega_qubit_range, omega_cavity_range, omega_drive_range, c_op_params, g, N)
...
class JaynesCummingsParameters:
    ''' interface to ssjcm class for unpacking parameters and reparametrising'''
...
def draw_qps(sys, type='Q', ininterval=50, contno=100, save=False, infigsize=(6, 6),  xvec=np.linspace(-8, 7, 50), yvec=np.linspace(-10, 4, 50), form='mp4'):
    ...
    def animate(i):
        axes.cla()
        plt.cla()
        plt.title(sys.important_range_name + ': %d'%W[i][0]) # SYSTEM SPECIFIC
    ...
    if save and len(W)!=1:
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            if form=='mp4':
                anim.save('qp_anim.mp4', fps=30, extra_args=['-vcodec', 'libx264'])
            if form=='gif':
...
# omega_cavity_range, omega_qubit_range, N_qubits, N_cavity_modes, c_op_params, g
...
carmichael_system_params = JaynesCummingsParameters(5, 100, np.linspace(90, 110, 42), 100, [1, 1], 10, 40).params()

carmichael_sys = SteadyStateJaynesCummingsModel(*carmichael_system_params)

draw_qps(carmichael_sys, 'W', 50, 100, True)
# plot_qps(carmichael_sys, 'Q')
# plot_exp(carmichael_sys, 'iphnum')
...
def draw_figs(estart, estep, estop, detstart, detstep, detstop):
    ...
    def animate(frame):
        system = SteadyStateJaynesCummingsModel(*JaynesCummingsParameters(estart+frame/(1/espace), 100, 100, np.linspace(detstart, detstop, 1/detspace), [1, 1], 10, 10).params())
    ...
    anim = animation.FuncAnimation(fig, animate, init_func=init, frames=int(estop), interval=20, blit=False, repeat=False)

    plt.show()

#INTERESTING
# carmichael_system_params = JaynesCummingsParameters(5, np.linspace(60, 140, 20), 100, 100, [1, 0.8], 10, 40).params()
