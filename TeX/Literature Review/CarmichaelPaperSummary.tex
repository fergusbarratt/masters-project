...
\begin{figure}[h]
	\begin{minipage}{.3\linewidth}
		...
		\subcaption{From density matrix calculations: IC photon number expectation, g = 25, $\kappa = 0.5$}
		\label{fig:densp}
	\end{minipage}%
	\begin{minipage}{.3\linewidth}
		\centering
		\includegraphics[width=0.8\textwidth]{Images/MeanFieldPhotonNumber.png}
		\subcaption{From meanfield calculations: IC photon number $n \approx |\alpha|^2$, g = 25, $\kappa$ = 0.5}
		\label{fig:meanfieldp}
	\end{minipage}%
	\begin{minipage}{.3\linewidth}
		\centering
		\includegraphics[width=1\textwidth]{Images/Q-Bifurcation.png}
		\subcaption{Bifurcation in the Q function for E from 0 to 1, g = 0.5,$ \kappa = 0.01$, $\Delta \omega$ = 1. From quantum results}
		\label{fig:Qbirfc}
	\end{minipage}
...
\implies \alpha &= -i\Epsilon \frac{1}{\kappa-i\Delta\omega+\frac{g^2(\frac{\gamma}{2}+i\Delta\omega)}{\frac{\gamma^2}{4}+
\Delta\omega^2+2g|\alpha|^2}}
\label{eq:alphadetdiss}
\end{align}
